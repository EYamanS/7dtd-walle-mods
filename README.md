...
- **Shared containers (multiplayer)**: multiple players can open the **same chest at the same time**, with live-syncing windows — no more "container in use". Built on the game's own shared-lock system (the one traders already use). Sharing for world loot, workstations and dropped bags exists as **experimental opt-in scopes** (off by default) in the config; quest containers always stay vanilla.
